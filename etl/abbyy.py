...
class abbyy(object):
    ...
    def connect(self):
        ...
        self.conn = httplib.HTTPConnection("parserservicebackup.abbyy.com", timeout=60)        
        self.conn.request("GET", "/api/server/markupLanguages", "", {"Authorization": "Basic U2NhbkV4Om1RM1BvNUJq"}) #U2NhbkV4Om1RM1BvNUJq  <-> BASE64 (ScanEx:mQ3Po5Bj)
    ...
    def sendSourceText(self,txtSource):
        ...
        #ABBYY Rest API was changed
        #params = '{"taskParameters": {"Source":{"Extension":".txt","Contents":' + str(toByteArray(txtSource)) + '},"UserCredentials":{"Login":"ScanEx","Password":"mQ3Po5Bj","IsCustomUser":true},"SourceLanguage":"","MarkupLanguage":"neutral","XmlParameters":["All"],"Topics":[],"Ontologies":["All"],"ProcessTimeout":1000000}}';
        ...
        headers = {"Authorization": "Basic U2NhbkV4Om1RM1BvNUJq","Content-type": "application/json; charset=utf-8","Accept": "*/*"}
        #print params
        #print headers
        #ABBYY Rest API was changed
    ...
    def isReady(self):
        ...
        self.conn.close()
        self.conn = httplib.HTTPConnection("parserservicebackup.abbyy.com", timeout=60)
        #print len(self.currentTaskId), self.currentTaskId
        self.conn.request("GET", "/api/tasks/" + self.currentTaskId, '', {"Authorization": "Basic U2NhbkV4Om1RM1BvNUJq","Content-type": "application/json; charset=utf-8","Accept": "*/*"})
        response = self.conn.getresponse()
        if(response.status != 200):
    ...
    def extractGeoOntologyObjects(self,xmlInput):
		...
		#with codecs.open('C:/Scanex/Operative/NewsRss/Test/' + str(len(xmlInput)) + '.xml', 'w', encoding = 'utf8') as f:
		#	f.write(xmlInput)
		#print(ontoObjects)
    ...
    def getCandidateNounPhrases(self,text):
        try:
            ...
            print "Error: ",sys.exc_info()[0],sys.exc_info()[1],traceback.format_exc()
            if(self.conn != None):
                self.disconnect();
            return []
