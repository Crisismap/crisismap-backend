...
def classify(type, file, lang):
  ...
  news = dbScanex.LoadUnclassifiedNews(type)
  for fn in news:
    #print fn["title"]
    #time.sleep(5)
    predict = mincl.predict((fn["title"], fn["body"]), model)[0]
    print predict
    dbScanex.UpdateNewsClass(fn["id"], predict)
    #return
  return
...
def classify_all():
  for key in classificators:
    try:
      classify(key, 'c:/Scanex/Operative/NewsRss/data/' + classificators[key][0], classificators[key][1])
    except:
