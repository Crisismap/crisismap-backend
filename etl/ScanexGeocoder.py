...
def getLocation(locationStr):
  try:
    SCANEX_GEOCODING_URL = u"http://geocode.kosmosnimki.ru/GeoSearch.ashx?UseOSM=1&IsStrongSearch=1&SearchString="
    #etostrokakotorayadolzhnavernutpustoyrezultat
    doc = urllib2.urlopen(SCANEX_GEOCODING_URL + urllib.quote(locationStr.encode("utf-8"))).read()
    ...
    print "Error parsing JSON output from Scanex: " + str(sys.exc_info()[1])
    return None;
