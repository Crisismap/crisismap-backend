  ...
  try:
    ...
    for entry in fires_lenta.entries:
      ...
      if hasattr(entry,'geo_long'):
        db.insert_news(entry, str_date, True)
        article = db.get_latest_news_article();
        
        db.StoreArticleLocations(article,[(entry.geo_long, entry.geo_lat)],'');
      else:
        if not hasattr(entry, 'guid'):
          entry.guid = entry.link
        db.insert_news(news_sources[url], entry, str_date, False)
    #db.close();
  except Exception, e:
